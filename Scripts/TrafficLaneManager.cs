...
public class AIVehicleLaneData {
	...
	public AIVehicleLaneData(int InVehicleID, float InVehicleProgress = 0f, float InSpeed = 1f, int InCalculationMode = 4)
	{
		...
		CalculationMode = InCalculationMode; // Spawn in as calc mode 4 (invalid state) so it'll act as if it's just entered whatever calc mode is appropriate for its distance
	}
}
...
public class TrafficRoadData {
	...
	public List<TrafficLaneData> Lanes = new List<TrafficLaneData>();
}
...
public class VehicleTrafficData {
	...
	public VehicleTrafficData (GameObject InObj, List<Transform> InWheels = default(List<Transform>), AudioSource InHornAudio = null, Rigidbody InRigidbody = null, AICarHandler InCarHandler = null)
	{
		VehicleObj = InObj;
		Wheels = InWheels;
...
public class MapTrafficData {
	...
	public List<TrafficRoadData> RoadData = new List<TrafficRoadData>();
}
...
public class TrafficLaneManager : MonoBehaviour {
	...
	public float AveragePlayerMass = 1000f;

	[Tooltip("When true traffic will wait behind cars knocked off their rails, otherwise they'll smash through them like they don't exist")]
	public bool StopForOffRoadCars = true; // When true AI will wait behind cars knocked off road rather than smash through them
	...
	[Tooltip("When enabled vehicles will never raycast and their Y position is determined based on the road points and beziers only (Useful if the entire map is flat)")]
	...
	[Header("Map & Road Data")]
	public List<MapTrafficData> TrafficData = new List<MapTrafficData> ();

	[Header("Vehicle Prefabs")]
	public List<VehicleTrafficData> CarTemplates = new List<VehicleTrafficData>();
	private List<VehicleTrafficData> CarObjects = new List<VehicleTrafficData> ();
	...
	public void AutoPopulateCarTemplates()
	{
		...
		for(int i=0;i < VehiclePrefabs.Length;i++)
			CarTemplates.Add(new VehicleTrafficData((GameObject)AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(VehiclePrefabs[i]), typeof(GameObject))));
	...
	public void AutoFindLanes()
	{
		...
		for (int MapID = 0; MapID < TotalMaps; MapID++) {
			...
			for (int CatID = 0; CatID < CurMapTransform.childCount; CatID++) {
				...
				for (int RoadID = 0; TotalRoadID < TotalRoads; RoadID++,TotalRoadID++) {
					...
					TrafficData [MapID].RoadData.Add (new TrafficRoadData ());
					TrafficRoadData CurRoadData = TrafficData[MapID].RoadData[TotalRoadID];
					...
					for (int LaneID = 0; LaneID < TotalLanes; LaneID++) {
						Transform CurLane = CurRoad.GetChild (LaneID);
						CurRoadData.Lanes.Add (new TrafficLaneData ());
						...
						CurLaneBezierHandlerStart.GetLength(50, true);
					...
					List<TrafficLaneData> SortedLanes = new List<TrafficLaneData>();
					...
					for(int i=0;i < CurRoadData.Lanes.Count; i++){
						...
						for(int CompareLaneId=0;CompareLaneId < CurRoadData.Lanes.Count; CompareLaneId++){
							...
							foreach(TrafficLaneData SortedLane in SortedLanes)
		...
		for (int MapID = 0; MapID < TotalMaps; MapID++) {
			for (int RoadID = 0; RoadID < TrafficData [MapID].RoadData.Count; RoadID++) {
				TrafficRoadData CurRoad = TrafficData[MapID].RoadData [RoadID];
	...
	public void SpawnRandomTrafficVehicle(int ForceRoad = -1, int ForceLane = -1)
	{
		...
		TrafficRoadData SelectedRoad = ActiveMapTrafficData.RoadData [RandomRoad];
		...
		TrafficLaneData SelectedLane = SelectedRoad.Lanes [RandomLane];
		...
		if (IsVehicleInProgress (SelectedLane, SpawnPosition - (DistanceBetweenVehicles / SelectedLane.Bezier.GetLength ()), 0, 2f) == BlockageType.None){
			...
			CarObjects.Add(new VehicleTrafficData(NewVehicle, NewVehicleRefData.Wheels, NewVehicleRefData.HornAudio, NewVehicleRefData.Rigidbody, NewVehicleRefData.CarHandlerScript));
			VehicleTrafficData NewCarObject = CarObjects[CarObjects.Count - 1];

			NewCarObject.VehicleRigidbody.detectCollisions = false;
			NewCarObject.VehicleRigidbody.mass *= (AveragePlayerMass / 6000f); // Decrease the AI vehicle masses to be in line with the player mass
			...
			if (VehicleCount > 0) {
				...
				for (int i = VehicleCount - 1; i >= 0; i--) {
					...
					if(CurVehicleLaneData.CalculationMode <= 2)
						CurCarData.CarHandler.TrafficAIUpdate();

					if(!CurCarData.IsOnRoad){
						if(CurVehicleLaneData.TimeUntilNextDistanceCheck <= 0f){
							CurVehiclePosition = CurCarData.CarHandler.transform.position;
						...
						// Smart AI still needs AI to continue their background logic ready for when they rejoin the road
						if(TrafficActionWhenHit != ActionWhenHit.SmartAIDynamicallyRejoin){
									...
									if(CurBlockageType == BlockageType.PlayerSiren){
										...
										if(CurRoad.IntersectionID < 0){
											...
											if(LaneID <= 0){
												...
												TrafficLaneData LeftLane = CurRoad.Lanes[LaneID - 1];

												if(CanJoinLane(LeftLane, CurVehicleLaneData, DistanceBetweenVehicles, false)){
													// Add the vehicle into the new lane
													LeftLane.ActiveAIVehicles.Add (new AIVehicleLaneData (CurVehicleLaneData.VehicleID, CurVehicleLaneData.VehicleProgress, CurVehicleLaneData.VehicleSpeed, CurVehicleLaneData.CalculationMode));
					...
					if (CurVehicleLaneData.VehicleProgress >= 1f) {
						...
						if (CurLane.JointLaneCount > 0) {
							...
							foreach (JointLaneData CurJointLaneData in CurLane.JointLanes) {
								TrafficLaneData CurJointLane = GetLane (CurJointLaneData);
							...
							if (IsLaneJoinable) {
								...
								TrafficLaneData NewLane = GetLane (CurLane.JointLanes [Random.Range (0, CurLane.JointLaneCount)]);

								// Add the vehicle into the new lane
								NewLane.ActiveAIVehicles.Add (new AIVehicleLaneData (CurVehicleLaneData.VehicleID, 0f, CurVehicleLaneData.VehicleSpeed, CurVehicleLaneData.CalculationMode));
	...
	public float UpdateAICalculationMode(AIVehicleLaneData LaneData, TrafficLaneData CurLane, Vector3 Position)
	{
		...
		VehicleTrafficData CurCarData = CarObjects[LaneData.VehicleID];
		...
		if (Distance >= 40000f * DeviceTierAdjustment) {
			if (LaneData.CalculationMode != 3) {
				switch(LaneData.CalculationMode)
				{
					...
					case 2: CalcMode2Count--; break;
					case 1: CalcMode1Count--; break;
					case 0: CalcMode0Count--; break;
				}
			...
			if(LaneData.CalculationMode != 2){
				...
				if(CurCarData.IsOnRoad){
					...
					CurCarData.CarHandler.SetWheelCollidersActive(true);
				}
			} else {
				if(CurCarData.IsOnRoad){
	...
	public TrafficLaneData GetLane(JointLaneData LaneData)
	{
	...
	public bool CanJoinLane(TrafficLaneData NewLane, AIVehicleLaneData CurVehicleLaneData, float CarDistance, bool IsRoadChange = false)
	{
		...
		if (IsRoadChange) {
			TrafficRoadData CurRoad = ActiveMapTrafficData.RoadData [NewLane.RoadID];
			...
			if (CurRoad.IntersectionID >= 0) {
				...
				if (NewLane.WaitForClearIntersection) {
					...
					for (int i = 0; i < CurRoad.LaneCount; i++) {
						TrafficLaneData ComparisonLane = CurRoad.Lanes [i];
		...
		if(CalculationMode <= 1){
			foreach(Transform CurTransform in TrafficMonitoredTransforms)
			{
				...
				if(MaxProgress >= 1f){
					...
					{
						TrafficLaneData CurJointLane = GetLane(SelectedLane.JointLanes[JointLaneId]);
	...
	private BlockageType GetTrafficBlockState(TrafficLaneData SelectedLane, float MinProgress, float MaxProgress)
	{
		...
		for(int i=0;i < SelectedLane.TotalActiveAIVehicles;i++)
		{
			...
			if(StopForOffRoadCars || CarObjects[CurVehicleLaneData.VehicleID].IsOnRoad){
